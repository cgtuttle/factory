...
#=======================================================
	...
	class CSVCell
		attr_accessor :column, :row, :value, :field_name
	end

#=======================================================

	class CSVRow
		attr_accessor :row, :cells
		...
		def add_cell(column, name, value)
			cell = CSVCell.new
			self.cells << cell
		end
...
#=======================================================
	...
	def import_file(file_spec)
		if file_spec
			...
			csv_file.parse
			@parsed_file = csv_file.content_rows
			...
			@parsed_file.each_with_index do |row, row_num|				
				...
				row.each_with_index do |value, col_num|
					...
					@row << "#{@cell.value}, #{row_num}, #{col_num}, #{@import.id}, #{@tenant_id}" # append to row array
				end
				#-----------------------------------------------------------
	
				@rows << "(#{@row.join("), (")})" # append array element delimiter
				@row_nums << "(#{@import.id}, #{row_num	}, #{@tenant_id})"
			end
			#-------------------------------------------------------------
...
#=======================================================
	...
	def update_cells_import_row_id # Find the import_row.id for the foriegn key in cells
		Rails.logger.level = 0
		sql = "UPDATE cells
			SET import_row_id = a.id
			FROM import_rows AS a 
			WHERE a.import_id = #{@import.id}
			AND cells.import_id = a.import_id
			AND cells.row_num = a.row_id"
		ActiveRecord::Base.connection.execute(sql)
		Rails.logger.level = 0
	end		
